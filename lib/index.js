...
function setupTimeout(data, callback) {
  ...
  data.pingTimeout = setTimeout(function() {
    ...
    // we may need to do _something_ more creative here because just setting
    // pingTimedOut and checking for it later doesn't work with one failure
    // condition calls to subscribe() hang (never call their callback). This
    // timeout has no effect if we're stuck in that state. I'm seeing this
    // against one of my servers, but only when the collection I try to
    // subscribe to is meteor.loginServiceConfiguration

    // Eli suggests throwing an exception. That's probably a good idea.
  }, data.timeout);
  callback(null, data);
}
